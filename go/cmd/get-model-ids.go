...
var getModelIds = &cobra.Command{
	Use: "get-model-ids",
	Short: "Extracts model ID as JSON from excel sheet",
	Run: func(cmd *cobra.Command, args []string) {
		...
		modelIdToRowIds := make(map[string][]int)
		...
		for i := 1; i <= maxRow; i++ {
			...
			dynamicKey := row.GetCell(videos.ColumnIdx.ModelID).Value
			modelIdToRowIds[dynamicKey] = append(modelIdToRowIds[dynamicKey], i)
		}


		prettyJson, err := json.MarshalIndent(modelIdToRowIds, "", "  ")
		if err != nil {
